...
import api from "../../config/api"; // <-- use your existing axios instance
...
export default function UserHistory() {
  ...
  // -------- replaced static rows with fetched rows ----------
  const [rows, setRows] = useState([]);        // actual rows displayed (mapped)
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  // Fetch all borrows and map to the UI shape
  useEffect(() => {
    const fetchHistory = async () => {
      setLoading(true);
      try {
        const res = await api.get("/borrow/borrow/"); // backend endpoint
        // assume res.data is an array of BorrowDetailResponse
        const mapped = (res.data || []).map((b) => ({
          // keep id string similar to previous UI style if needed
          id: `BRW-${b.borrow_id}`,
          borrow_id: b.borrow_id,
          book: b.book_title,
          user: b.user_name,
          type: capitalize(b.borrow_status),           // e.g., Borrowed / Returned
          requestStatus: capitalize(b.request_status), // Pending / Accepted / Rejected
          borrowedOn: b.borrow_date,
          dueOn: b.return_date,
          returnedOn: (b.borrow_status && String(b.borrow_status).toLowerCase() === "returned") ? b.return_date : "",
  ...
  // New filter chips requested:
  // All | Borrowed | Returned | Pending | Accepted | Rejected
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      // filtering logic:
      // - if typeFilter is Borrowed/Returned -> filter by borrow_status (r.type)
      // - if typeFilter is Pending/Accepted/Rejected -> filter by request_status (r.requestStatus)
      // - All -> no filter
      ...
      if (typeFilter !== "All") {
        ...
        if (tf === "borrowed" || tf === "returned") {
          ...
          // pending / accepted / rejected => check requestStatus
              ...
              <li key={r.id} className="py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    {/* show request status under user on mobile */}
                    <p className="text-xs text-gray-500 mt-1">Request: <span className="font-medium">{r.requestStatus || "—"}</span></p>
                  </div>
                  <button
                    type="button"
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-white">
                  <tr className="text-left">
                    ...
                    <th className="py-3 px-4 min-w-[140px]">Request Status</th>
          ...
          <div className="absolute inset-0 flex items-center justify-center px-4">
            <div className="w-full max-w-lg mx-4 rounded-lg bg-white shadow-lg border border-gray-200 opacity-0 translate-y-2 animate-[popIn_.22s_ease-out_forwards]">
              ...
              <div className="px-4 md:px-6 py-4 md:py-5 space-y-3 text-sm">
                ...
                <div className="flex justify-between gap-4"><span className="text-gray-500">Request Status</span><span className="font-medium text-gray-800">{detail.requestStatus || "—"}</span></div>
